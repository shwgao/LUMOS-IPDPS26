...
def tfrecord_to_npy(tfrecord_path, output_folder):
    # 创建输出文件夹（如果不存在）
    ...
    # 定义解析函数
    feature_description = {
        'feature_name': tf.io.FixedLenFeature([], tf.float32),  # 根据你的 TFRecord 文件结构修改
    }
    ...
    for i, record in enumerate(parsed_dataset):
        np.save(os.path.join(output_folder, f'record_{i}.npy'), record['feature_name'].numpy())
